...
app.get('/privacy-policy', (req, res) => {
  res.send(`
    ...
    <p>
      Aplikacja Sellmo zbiera wyłącznie publicznie dostępne dane z komentarzy
      pod transmisjami na żywo na stronie Facebook, w tym imię i nazwisko
      użytkownika oraz treść komentarza.
    </p>

    <h2>Jak wykorzystujemy dane?</h2>
    <p>
      Dane są przechowywane w MongoDB Atlas i używane wyłącznie do obsługi
      zamówień oraz wyświetlania ich w panelu administratora.
    </p>
...
app.post('/webhook', async (req, res) => {
  ...
  console.log(
    '📩 Otrzymano zdarzenie z Facebooka:',
  ...
  for (const entry of body.entry) {
    ...
    for (const change of changes) {
      ...
      if (value.item === 'comment' && value.verb === 'add') {
        ...
        console.log(
          `💬 Komentarz pod live ${liveVideoId}: "${message}" od ${userName}`
        );
        ...
        try {
          ...
          const replyMessage =
            process.env.ORDER_REPLY_MESSAGE ||
